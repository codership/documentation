...
Just below each question is further categorization of the question: the minimum experience level of the person who might be interested |---| if you are new to database clusters, you might want to skip the Intermediate ones; and the type of person who might be interested in such a question (for example, DBAs, business managers).
...
.. rst-class:: list-abstract
   ...
   Galera Cluster is a write-set replication service provider in the form of the *dlopenable* library. It provides synchronous replication and supports multi-primary replication. Galera Cluster is capable of unconstrained parallel applying (that is, "parallel replication"), multicast replication and automatic node provisioning.
...
.. rst-class:: list-abstract
   ...
   With Galera, if one primary fails, the cluster continues and users can continue to write and read on other nodes. With standard MySQL replication, if the primary fails, users cannot write until it is restored or replaced - which can involve manual intervention and take good bit of time.
...
.. rst-class:: list-abstract
   ...
   Yes, it works just fine. Through Amazon's EC2 environment, you can create multiple instances, virtual servers running the Linux operating system--any distribution is fine. After the instances are created, you would log into each instance and install MySQL or MariaDB and Galera, as well as configure them. On AWS, you will have to set inbound security rules to allow the instances to communicate with each.
...
.. rst-class:: list-abstract
   ...
   Major companies all over the world have implemented Galera to protect their data and secure their application and service availability. Galera Cluster is included in Debian Linux distributions and it is the most used high-availability solution for OpenStack Cloud platform, according to their survey.
...
.. rst-class:: list-abstract
   ...
   For comprehensive training courses on Galera and related software (for example, load balancers), check the web sites of our partners (for example, MariaDB, FromDual, Severalnines). For a list of all of them, along with links to their sites, see the `Support Partners <https://galeracluster.com/support/#support-partners>`_
...
.. rst-class:: list-abstract
   ...
   Since end-users won't do anything different from what they already do when adding and changing data in the database, there's nothing new for them to know. As for database developers, they mostly need to be aware that they can use only InnoDB tables. They can't use other storage engines. If they do not already, they might want to learn about the features of InnoDB so they can take advantage of them (for example, transactions).
...
.. rst-class:: list-abstract
   ...
   You can find many articles on Galera and related software on our `blog <https://galeracluster.com/category/blog/>`_. These are mixed in with information on conferences and press releases, so you will have to scroll through the list of articles. Some of our partners regularly publish articles on various aspects of Galera: `MariaDB <https://mariadb.com/resources/blog/tag/galera/>`_, `Severalnines <https://severalnines.com/blog/top-mysql-galera-cluster-resources>`_, and `FromDual Articles <https://www.fromdual.com/search/node/galera>`_.
...
.. rst-class:: list-abstract
   ...
   In a way, Galera is a behind-the-scene feature. It is seamless and very much hidden from users. A developer may access any node in a Galera cluster to change table schemata.
   ...
   Users would insert or change data in a database in the same as they would on a stand-alone database server not using Galera or replication. There is no extra login requirements, interfaces, or methods to use a database running on Galera Cluster. Users will be unaware that you are using Galera Cluster |---| other than maybe noticing that your database is much more dependable.
...
.. rst-class:: list-abstract
   ...
   Consulting is usually done remotely. However, if you require in-person, on-site work, there will be extra charges (such as travel and accommodation expenses).
...
.. rst-class:: list-abstract
   ...
   It is potentially easy. There are a few things to consider, changes you may need to make.

   First, you will have to migrate all of your tables to InnoDB. Although MySQL and MariaDB offer multiple storage engines, Galera only allows InnoDB tables. You'll also have to address how changing to InnoDB will affect your applications.
   ...
   Last, you may want to make some changes to your hardware. For one, if you have only two servers, you should add a third. Although it is not necessary, it is recommended that all servers used be the same or fairly equal in resources.

   Basically, if you are already using the latest database software and only InnoDB tables, implementing Galera will be very easy. Otherwise, implementing Galera will require some thought and effort. However, the result will mean a much better cluster:  all servers will be the same for easier maintenance and better performance; they'll be running the latest software, which will provide advantages; and the data will be better protected and will have high availability.
...
.. _`faq-change-apps`:
.. rst-class:: sub-heading
.. rubric:: Do we have to Adjust our Databases or Custom Applications (for example, PHP Programs)?
...
.. rst-class:: list-abstract
   ...
   If you are already using MySQL or MariaDB, along with some custom applications |---| such as programs written in PHP, Perl, Ruby, or another language, that interface with your databases |---| you shouldn't have to make any changes to your software.

   If you are currently using standard MySQL Replication, and your applications connect with specific nodes for writes and others for reads, you probably won't have to do that. Instead, you can write and read to the same nodes. As for load balancing, you could add a load balancer like MaxScale and then direct all traffic to the load balancer and it will direct the traffic for the best performance.
...
.. rst-class:: list-abstract
   ...
   The minimum number of nodes required for a cluster is two. However, a minimum of three nodes is recommend. In a two-node cluster, if one node fails or is taken down for maintenance, the other node will stop since another node is required. There is a work around for two-node cluster issues: see :doc:`Two-Node Clusters <./kb/two-node-clusters>`.
...
.. rst-class:: list-abstract
   ...
   The best equipment you can afford to buy, the better. If you are using virtual servers like those through Amazon's AWS, you do not need to be concerned about most of these equipment factors. You will just need to allow your servers enough memory and storage space.

   However you build your server nodes, it is best that they be equal in all ways: physical and virtual equipment; operating system configuration; software installation.
...
.. rst-class:: list-abstract
   ...
   For high-traffic clusters, to prevent one node from being overwhelmed with write and read queries, you may want to use a load balancer. Galera Cluster does not include this feature. However, we could use MariaDB's MaxScale, ProxySQL, or some other such load balancer.
...
.. rst-class:: list-abstract
   ...
   The primary focus of Galera Cluster is data consistency across the nodes. This does not allow for any modifications to the database that may compromise consistency. For instance, the node rejects write requests until the joining node synchronizes with the cluster and is ready to process requests.
...
.. rst-class:: list-abstract
   ...
   Standard MySQL replication uses the binary log for replicating. However, Galera does not use the binary log. Nevertheless, there may be situations in which you might want to use point-in-time recovery methods to restore tables or data since the last backup.

   You might also want to attach an asynchronous replica to one of your nodes, using standard MySQL replication and set it on a delay, using time delayed replication (present in MySQL and MariaDB Server). This can also help with recovering tables and data lost since the last backup was made.
...
.. rst-class:: list-abstract
   ...
   Although it does not happen often, there are several reasons a Galera Cluster might crash. Below is a list of them, grouped by type of cause:
   ...
   **Storage Engine Causes**
   ...
   - Creating or dropping tables that do not have a primary key
...
.. rst-class:: list-abstract
   ...
   To alleviate this problem, you would make sure that all of the servers the same physically (that is, amount of RAM, types of network interfaces), or at least have close the same amount of resources available. You would also use a load balancer (for example, MariaDB MaxScale, ProxySQL) to make sure one node is not overloaded with traffic.
...
.. rst-class:: list-abstract
   ...
   The Galera software is licensed under the GNU General Public License, version 2 (see `GPL vs. 2 <https://www.gnu.org/licenses/old-licenses/gpl-2.0.en.html>`_). It is open-source software, which can be found at GitHub (see `Codership Github <https://github.com/codership>`_).
